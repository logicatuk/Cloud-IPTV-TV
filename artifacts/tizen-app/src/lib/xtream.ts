...
export async function verifyCredentials(
  ...
  try {
    ...
    const data = await res.json();
    if (data?.user_info?.auth === 1 || data?.user_info?.auth === "1") return { valid: true };
    if (data?.user_info?.auth === 0 || data?.user_info?.auth === "0") {
      return { valid: false, message: "Wrong username or password" };
    }
    return { valid: true };
  } catch (e: unknown) {
    if (e instanceof Error && e.name === "AbortError") return { valid: false, message: "Connection timed out" };
...
export async function getLiveStreams(creds: XtreamCredentials, categoryId?: string): Promise<XLiveStream[]> {
...
export async function getVodStreams(creds: XtreamCredentials, categoryId?: string): Promise<XVodStream[]> {
  const extra: Record<string, string | number> = {};
  if (categoryId && categoryId !== "all") extra.category_id = categoryId;
  return xFetch<XVodStream[]>(apiUrl(creds, "get_vod_streams", extra));
}

export async function getVodInfo(creds: XtreamCredentials, vodId: number): Promise<XVodInfo> {
  return xFetch<XVodInfo>(apiUrl(creds, "get_vod_info", { vod_id: vodId }));
}

export function buildVodStreamUrl(creds: XtreamCredentials, streamId: number, ext: string): string {
...
export async function getSeriesList(creds: XtreamCredentials, categoryId?: string): Promise<XSeriesStream[]> {
  const extra: Record<string, string | number> = {};
  if (categoryId && categoryId !== "all") extra.category_id = categoryId;
  return xFetch<XSeriesStream[]>(apiUrl(creds, "get_series", extra));
}

export async function getSeriesInfo(creds: XtreamCredentials, seriesId: number): Promise<XSeriesInfo> {
  return xFetch<XSeriesInfo>(apiUrl(creds, "get_series_info", { series_id: seriesId }));
}

export function buildEpisodeStreamUrl(creds: XtreamCredentials, episodeId: string | number, ext: string): string {
  return `${creds.host}/series/${creds.username}/${creds.password}/${episodeId}.${ext || "mkv"}`;
}

export async function getAccountInfo(creds: XtreamCredentials): Promise<{
  username: string;
  status: string;
  expDate: number | null;
  isTrial: boolean;
